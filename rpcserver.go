...
var (
	...
	ErrNoAccountSupport = btcjson.Error{
		...
		Message: "wallet does not support non-default accounts",
	}
...
// account names, as account support is currently missing from wallet.
...
// websocket connections for any number of wallet clients.
...
// method.  This may be a request that is handled directly by wallet, or
// a chain server request that is handled by passing the request down to pointcoind.
//
...
func (s *rpcServer) PostClientRPC(w http.ResponseWriter, r *http.Request) {
	...
	case "stop":
		s.Stop()
		resp = makeResponse(raw.ID, "wallet stopping.", nil)
	default:
...
func (b pointcoindConnected) notificationCmds(w *Wallet) []btcjson.Cmd {
...
out:
	for {
		...
		case n := <-s.chainServerConnected:
			s.enqueueNotification <- pointcoindConnected(n)
...
var rpcHandlers = map[string]requestHandler{
	...
	// Reference methods which can't be implemented by wallet due to
...
// unsupported by wallet due to design differences.
...
// requests from the same client, on the result of a pointcoind RPC can result is too
...
// information about the current state of wallet.
// exist.
func GetInfo(w *Wallet, chainSvr *chain.Client, icmd btcjson.Cmd) (interface{}, error) {
	// Call down to pointcoind for all of the information in this command known
	// by them.
...
// appear in the blockchain, or any tx that has arrived in the pointcoind mempool).
...
//                  default: false.
// Since wallet doesn't implement account support yet, only the default account ""
// will be returned
...
func SignRawTransaction(w *Wallet, chainSvr *chain.Client, icmd btcjson.Cmd) (interface{}, error) {
	...
	// TODO(oga) really we probably should look these up with pointcoind anyway
	...
	// querying pointcoind with getrawtransaction. We queue up a bunch of async
